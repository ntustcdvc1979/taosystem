...
function formatEvents(events) {
  ...
  return events
    .map((e) => {
      ...
      return `- ${dateStr}【${e.type || "未分類"}】${e.name}`;
    })
    .join("\n");
}
...
3. recommendedActivity：從上面的近期活動中挑選最適合這位對象目前狀況的「一個」活動，填「日期 活動名稱」；若近期沒有合適的活動就填空字串。活動類型與適合對象：廣結善緣＝接觸初期、未求道或反應冷淡者，先建立關係；獻供＝壇務性質、氣氛輕鬆，適合帶新朋友來廣結善緣或維繫關係；求道＝已建立關係、時機成熟的未求道者參加求道禮；成全＝已求道者深化的研究班或成全課程；法會＝已求道、關係穩定者；幹訓＝較資深、班程有基礎、有心承擔的人才。做法（method）中請一併說明如何鋪陳邀約這個活動。`;
...
使用者是負責成全的同修，會向你詢問名單相關的建議（例如：誰適合邀約參加法會、某位對象下一步怎麼做、整體優先順序等）。請根據名單內容給出具體、可執行的建議；規劃時參照近期活動——依對象目前的狀況建議適合參加哪個活動、如何鋪陳邀約（廣結善緣＝接觸初期或反應冷淡者；獻供＝氣氛輕鬆，適合帶新朋友或維繫關係；求道＝時機成熟的未求道者；成全＝已求道者的研究班；法會＝已求道、關係穩定者；幹訓＝較資深、有心承擔者）。回答保持精簡，不要長篇大論。記得一律用代號指稱名單上的人。`;
