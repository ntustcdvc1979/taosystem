...
系統內的顯示與資料庫儲存都保留**完整姓名**；只有在把資料送給 AI 分析時，**姓名、聯絡人**才會自動遮罩：王小明 → 王○明、王明 → 王○、歐陽小明 → 歐○○明。你在 AI 聊天室輸入的訊息若包含名單上的姓名，送出前也會自動換成遮罩版。DeepSeek API 收到的資料不含完整姓名。

自行留意：**背景、備註等自由文字欄位不會自動處理**，若不希望某些資訊（例如學校、單位全名）送給 AI，請勿寫在這些欄位，或送出前先修改。

## AI 成全建議（DeepSeek）

AI 功能使用 **DeepSeek**（`deepseek-chat` 模型，OpenAI 相容 API），有兩種使用方式：

### 1. 單一對象建議

表格每列的「AI 建議」按鈕：開啟視窗後可以（選填）輸入**一句話的策略方向**（預設帶入該對象目前的策略），AI 會依你的方向與該對象去識別化後的背景、成全狀況、活動紀錄，產生「策略」與「具體做法」，滿意的話一鍵套用回欄位。

### 2. AI 成全助手（聊天室）

登入後右下角有藍色「AI」浮動按鈕，點開是一個聊天視窗。這位助手**能看到整份名單**（每一位的姓名、聯絡人都已遮罩），可以自由對話詢問，例如：

- 「哪幾位適合邀約下個月的法會？」
- 「王○明最近反應冷淡，接下來怎麼做比較好？」
- 「幫我排一下這份名單的成全優先順序。」

「清除對話」可重新開始；對話紀錄只存在當前頁面，重新整理即消失，不會寫入資料庫。

### 設定共用 API Key（管理員做一次）

所有白名單成員共用一把 DeepSeek API Key，存在 **Firestore 的 `config/ai` 文件**，由安全規則的 Email 白名單保護——只有登入且在白名單內的人讀得到，使用者不需要各自輸入。

1. 到 [platform.deepseek.com](https://platform.deepseek.com/) 註冊、儲值並建立 API Key。
2. 到 Firebase Console →「Firestore Database」→「資料」→ 新增集合：
   - 集合 ID：`config`
   - 文件 ID：`ai`
   - 欄位：`deepseekApiKey`（字串），值填入你的 API Key
