...
姓名、乾坤（乾／坤）、系級、背景（自由文字描述，例如：熱音社社員、大四家教、系排球隊）、備註、聯絡人、成全狀況（固定選項：未求道／已求道／法會畢／新民班畢／至善班畢／行德班畢／崇德班畢／人才培訓班畢／講培班畢／講師／未確定）、策略、做法。

另外每個人有兩份獨立的紀錄，都透過表格中每列的按鈕開啟，與新增/編輯名單的表單分開管理：
...
背景、備註、策略、做法、活動紀錄這些自由文字欄位裡若提到名單上的人（不論是成全對象或聯絡人），送出前一樣會換成代號。為了讓實際寫法也能被抓到，對照表額外涵蓋：
...
表格每列的「AI 建議」按鈕：開啟視窗後可以（選填）輸入**一句話的策略方向**（預設帶入該對象目前的策略），AI 會依你的方向與該對象去識別化後的背景、成全狀況、活動紀錄，產生「策略」與「具體做法」，滿意的話一鍵套用回欄位（套用時已換回真名）。
