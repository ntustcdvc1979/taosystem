...
某些標籤代表較敏感的身分（例如「團內幹部」），只要一個對象**有這個標籤**，他的**整張卡片**預設就不會出現在名單裡（不是只藏標籤本身），也不會被搜尋到。工具列的「**顯示內部標籤**」按鈕可切換顯示/隱藏；旁邊會顯示目前隱藏了幾筆，避免誤以為資料不見了。要調整哪些標籤會觸發整卡隱藏，編輯 [src/main.js](src/main.js) 裡的 `HIDDEN_TAGS` 陣列即可（這只是畫面顯示層級的過濾，AI 建議仍會看到完整名單以做出更好的判斷）。
...
工具列的「**活動管理**」按鈕可登錄近期活動：開始日期、結束日期（選填，留空表示單日活動，例如兩天一夜的幹訓）、名稱、類型（**廣結善緣／求道／成全求道／法會／幹訓**），存在 Firestore 的 `events` 集合，白名單成員都能維護。
