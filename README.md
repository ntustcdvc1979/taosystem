...
姓名、性別（乾／坤）、系級、背景（自由文字描述，例如：熱音社社員、大四家教、系排球隊）、聯絡人、成全狀況（固定選項：未求道／已求道／法會畢／新民班畢／至善班畢／行德班畢／崇德班畢／人才培訓班畢／講培班畢／講師／未確定）、策略、做法、推薦活動（由 AI 建議產生）。

名單以**卡片**呈現（依螢幕寬度自動排欄），長文字預設收合 3 行、點一下展開。另外每個人有兩份獨立的紀錄，透過卡片上的按鈕開啟，與新增/編輯名單的表單分開管理：

- **活動紀錄**（活動名稱、日期、反應）：參加過哪些活動、當下的反應。
- **聊天成全紀錄**（日期、內容）：每次聊天成全的日期與談了什麼。

兩者都會在產生 AI 建議時一併送給 AI 參考。

## 活動管理

工具列的「**活動管理**」按鈕可登錄近期活動：日期、名稱、類型（**廣結善緣／成全求道／法會**），存在 Firestore 的 `events` 集合，白名單成員都能維護。

AI 產生建議時會參照今天（含）以後的活動：單一對象建議會多產出「**推薦活動**」（依對象狀況挑選——接觸初期優先廣結善緣類型，已求道、關係穩定者推成全求道類型或法會），套用後顯示在卡片上；聊天室助手也能回答「誰適合參加哪個活動、怎麼鋪陳邀約」。
