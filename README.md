...
系統內的顯示與資料庫儲存都保留**完整姓名**；只有在把資料送給 AI 分析時才會去識別化：

- **姓名、聯絡人**：自動遮罩，例如王小明 → 王○明、王明 → 王○、歐陽小明 → 歐○○明。
- **背景、系級、備註、策略、做法、活動紀錄**：套用你在「詞彙遮罩設定」裡自訂的關鍵字替換（見下方）。

Anthropic API 收到的資料不含完整姓名，也不含你設定要遮罩的關鍵字。

### 詞彙遮罩設定（自訂關鍵字）

點右上角「**詞彙遮罩設定**」（或 AI 建議視窗裡的「管理詞彙遮罩」）可以自行新增/刪除要遮罩的關鍵字，例如系名縮寫、社團或班別的簡稱（「台科」「術德」之類）。每筆設定是「關鍵字 → 替換內容」：

- 有填替換內容：送給 AI 前會把關鍵字換成你填的內容（例：台科 → ○科大）。
- 替換內容留空：自動換成對應字數的「○」。

這份清單存在 Firestore 的 `deidentifyTerms` 集合，白名單內的人都可以直接在網頁上維護，不需要進 Firebase Console。**只影響送給 AI 的內容，網頁上的顯示與資料庫原始資料不會被改寫。**

## AI 成全建議

表格每列有「AI 建議」按鈕：會把該對象去識別化後的背景、系級、成全狀況、聯絡人、目前策略/做法與活動紀錄送給 Claude（Anthropic API），產生建議的「策略」與「具體做法」，滿意的話可一鍵套用回該對象的欄位。
