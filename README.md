...
這是一份**可還原的對照表**：AI 依代號回覆後，系統會自動把代號換回真名再顯示（例如 AI 說「建議代號3參加法會」，你看到的是「建議王小明參加法會」）。各欄位（背景、備註、策略、做法、活動紀錄）裡若出現名單上的人名，送出前也會一併換成代號。Anthropic 收到的資料完全不含真名。

自行留意：只有**名單上登記過的姓名/聯絡人**會被代換，若你在自由文字欄位寫了名單外的人名或學校、單位全名，不會自動處理，請自行斟酌。
