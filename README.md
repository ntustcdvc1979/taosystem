...
姓名在儲存進資料庫前會**自動遮罩**：王小明 → 王○明、王明 → 王○、歐陽小明 → 歐○○明。資料庫（Firestore）永遠不保存完整姓名，即使資料外洩也難以直接對應到特定個人。已含「○」的姓名視為已遮罩，不會重複處理。

自行留意：**備註、背景等自由文字欄位不會自動處理**，請避免在裡面寫入完整姓名、學號、電話等可直接識別個人的資訊。

## AI 成全建議

表格每列有「AI 建議」按鈕：會把該對象**去識別化後**的背景、系級、成全狀況、目前策略/做法與活動紀錄送給 Claude（Anthropic API），產生建議的「策略」與「具體做法」，滿意的話可一鍵套用回該對象的欄位。

使用前需要一把 Anthropic API Key：

1. 到 [console.anthropic.com](https://console.anthropic.com/) 註冊並建立 API Key（需綁定付款方式；每次產生建議約耗費數美分）。
2. 第一次點「AI 建議」時，把 Key 貼進對話框並儲存。
3. Key **只存在你自己瀏覽器的 localStorage**，不會上傳到任何伺服器、不會寫入程式碼或 git。每個使用者要在自己的瀏覽器各自設定一次；點「更換 API Key」可清除重設。

> 注意：因為這是純前端網站，API 呼叫直接從瀏覽器發出（使用 Anthropic 官方支援的 browser access 模式）。請不要把 Key 分享給白名單以外的人。
