...
某些標籤內容較敏感（例如「團內幹部」），預設不會顯示在卡片上、也不會被搜尋到。工具列的「**顯示內部標籤**」按鈕可切換顯示/隱藏。要調整哪些標籤預設隱藏，編輯 [src/main.js](src/main.js) 裡的 `HIDDEN_TAGS` 陣列即可（這只是畫面顯示層級的過濾，AI 建議仍會看到完整標籤以做出更好的判斷）。
