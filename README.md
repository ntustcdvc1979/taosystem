...
- **聊天成全紀錄**（日期、內容）：每次聊天成全的日期與談了什麼。

日期欄位皆為日曆選擇器。兩份紀錄都會在產生 AI 建議時一併送給 AI 參考。

## 活動管理

工具列的「**活動管理**」按鈕可登錄近期活動：日期、名稱、類型（**廣結善緣／求道／成全求道／法會／幹訓**），存在 Firestore 的 `events` 集合，白名單成員都能維護。
...
背景、策略、做法、活動紀錄、聊天成全紀錄、活動名稱這些自由文字裡若提到名單上的人（不論是成全對象或聯絡人），送出前一樣會換成代號。為了讓實際寫法也能被抓到，對照表額外涵蓋：
...
卡片上的「AI 建議」按鈕：開啟視窗後可以（選填）輸入**一句話的策略方向**（預設帶入該對象目前的策略），AI 會依你的方向與該對象去識別化後的背景、成全狀況、活動紀錄、聊天成全紀錄，加上近期活動清單，產生「策略」「具體做法」與「推薦活動」，滿意的話一鍵套用回欄位（套用時已換回真名）。
