...
工具列的「**活動管理**」按鈕開啟**月曆檢視**（一週一列、活動落在對應日期格，多日活動每一天都會出現）。可上一月／下一月／今天切換。點空白日期格會把該天帶入下方表單的開始日期，點月曆上的活動則載入表單編輯或刪除。月曆會補滿完整週，**當月前後鄰月的日期與活動也會以淺字顯示**，方便銜接看。每筆活動包含：開始日期、結束日期（選填，留空表示單日活動，例如兩天一夜的幹訓）、名稱、類型（**廣結善緣／獻供／求道／成全／法會／幹訓**，不同類型以不同顏色標示），存在 Firestore 的 `events` 集合，白名單成員都能維護。
