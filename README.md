...
姓名、性別（乾／坤，坤以紅色標示）、系級、標籤（可多個，逗號分隔）、背景、聯絡人、成全狀況（固定選項：未求道／已求道／法會畢／新民班畢／至善班畢／行德班畢／崇德班畢／人才培訓班畢／講培班畢／講師／未確定）、策略、做法、推薦活動（由 AI 建議產生）。
...
點月曆上的活動進入編輯後，下方會出現該活動的**邀約名單**，做成四個狀態框：**預定邀約／已邀約待回覆／已回覆可以／已回覆不行**，各框顯示人數，上方也有整體摘要。

- **加入**：在輸入框**打字**即可（會自動提示名單上還沒加入的人），選好狀態按「加入」或直接按 Enter。
- **改狀態**：把名字**拖到另一個框**。
- **移除**：點名字旁的 ×。
...
邀約狀況也會送給 AI 參考：已回覆不行的活動不會再推薦給同一個人，已回覆可以的則會著重後續陪同；聊天室助手也能回答「誰還沒回覆某活動」。每筆活動包含：開始日期、結束日期（選填，留空表示單日活動，例如兩天一夜的幹訓）、名稱、類型（**廣結善緣／獻供／求道／成全／法會／幹訓**，不同類型以不同顏色標示），存在 Firestore 的 `events` 集合，白名單成員都能維護。
...
**對話會自動保存**：存在 Firestore 的 `chatHistories/{你的 uid}`，每位使用者一份，安全規則限定**只有本人讀得到**（其他白名單成員也看不到你的對話）。重新整理、關掉瀏覽器或換裝置登入，紀錄都還在。按「清除對話」會一併把雲端那份清空。
...
> ⚠️ **不要**把 API Key 放進 `.env`、GitHub Secrets 或任何會被 build 進前端的變數——`VITE_*` 環境變數會直接寫進部署後的公開 JS 檔，任何人不用登入就能取走這把 Key。存在 Firestore 由白名單保護，是純前端架構下唯一不會公開 Key 的做法（Firebase 那組設定值可公開，性質不同）。
>
